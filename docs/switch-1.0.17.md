...
The three atlases, armor, glint, fire, clock, and compass reference images use RGBA data. Canvas composition requests an alpha-capable context, clears a destination slot before drawing, and disables interpolation so transparent source pixels replace rather than reveal the vanilla slot beneath them. The verified water and lava files remain in the baseline but are not replaced from Java or Bedrock input.
...
Input water, flowing water, lava, flowing lava, and cauldron water textures are intentionally skipped before decoding or animation/mipmap generation. The dimensions above document the vanilla Switch baseline files that remain in the output.
